...
class GasAlamController extends Controller
{
    ...
    {
        $gasalam = DB::table('vw_gas_alam as a')
            ->select('a.*');
    ...
    {
        $existing = GasAlam::where('tahun', $request->tahun)->where('pabrik', $request->pabrik)->where('id_sumber_emisi', $request->id_sumber_emisi)->first();
        if ($existing) {
