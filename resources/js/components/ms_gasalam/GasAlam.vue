<template>
    <div class="d-flex flex-column flex-column-fluid">
		...
		<div id="kt_app_toolbar" class="app-toolbar d-flex flex-stack py-4 py-lg-8">
			...
			<div class="d-flex flex-grow-1 flex-stack flex-wrap gap-2 mb-n10" id="kt_toolbar">
				...
				<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
					<!--begin::Title-->
					<h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Gas Alam</h1>
					<!--end::Title-->
					<!--begin::Breadcrumb-->
					<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
						...
						<!--end::Item-->
						<!--begin::Item-->
						<li class="breadcrumb-item text-dark">Gas Alam</li>
						<!--end::Item-->
					</ul>
					<!--end::Breadcrumb-->
				</div>
      ...
      <DxDataGrid
          id="gridContainerGasAlam"
          ref="dataGrid"
          key-expr="id"
          :data-source="dataSourceGasAlam"
  ...
  <ImportModalGasAlam @refreshGridParent="refreshGrid"></ImportModalGasAlam>
  <AddModalGasAlam @refreshGridParent="refreshGrid"></AddModalGasAlam>
  <EditModalGasAlam @refreshGridParent="refreshGrid" :detailgasalam="detailgasalam"></EditModalGasAlam>
</template>
...
import ImportModalGasAlam from "./ImportGasAlam.vue";
import AddModalGasAlam from "./AddGasAlam.vue";
import EditModalGasAlam from "./EditGasAlam.vue";

const is_exporting = false;
const storeGasAlam = new CustomStore({
  key: 'id',
  load(loadOptions) {
    ...
    return axios.post('/api/gasalam/loadgasalam', args)
                    .then(response => ({
...
export default {
    ...
    data() {
        return {
            dataSourceGasAlam: storeGasAlam,
            detailgasalam: []
        }
    },
    created() {
    ...
    methods: {
        ...
        addGasAlam(){
          $('#kt_add_modal_gas_alam').modal('show');
        },
        importGasAlam(){
          $('#kt_import_modal_gas_alam').modal('show');
        },
        allowDeleting(e) {
          return true;
        },
        ...
        deleteGasAlam(e) {
          ...
          result.then((dialogResult) => {
            if(dialogResult){
              ...
              axios.delete(`/api/gasalam/delete/${id}`).then(response=>{
                //refresh grid
                this.$refs.dataGrid.instance.state(null);
              }).catch(function (error) {
                console.error(error);
        ...
        editGasAlam(e) {
          ...
          let id = e.row.data.id
          $('#kt_edit_modal_gas_alam').modal('show');
          axios.get('/sanctum/csrf-cookie')
          axios.get(`/api/gasalam/edit/${id}`)
                .then(response => {
                    this.detailgasalam = response.data;
                })
                .catch(function (error) {
                    console.error(error);
                });
